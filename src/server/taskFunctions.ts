...
export async function createTaskForCurrentUser(input: { title: string; note?: string | null; dueDate?: string | null }) {
...
export async function listMainViewTasksForCurrentUser(input: { filter: TaskFilter; sort: TaskSort; today: string }) {
  await taskRepository.initialize();
